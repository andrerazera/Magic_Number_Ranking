...
if toggle1 == 'ON':
    ...
    def UltBalancoProcessado(ticker):
        # Function that returns the date from the last result processed in fundamentus
        # website of the 'ticker' declared and fills the 'ranking' df in the line of the ticker
        ...
        xpath= r"/html/body/div[1]/div[2]/table[2]/tbody/tr[1]/td[4]/span"   #xpath of the field "last processed result"
        data_ult_bal = driver.find_element_by_xpath(xpath).text #get the text from field in str - don't see a need to convert it into a date format
        ranking.loc[ranking[ranking['Papel'] == ticker].index[0], 'UltBalProc'] = data_ult_bal  #using .loc, find the index of the ticker and fill in a specific column for this date
        
    # ==== Call Function ====================
    for ticker in ranking['Papel'][0:100]:   #call function for the first 100 tickers
        UltBalancoProcessado(ticker)
