...
# %%
# ==== Add timestamp to ranking df ====================
# ===== Feature added 25/08/2021
from datetime import datetime
ranking['TimeStamp'] = datetime.now()
...
if toggle == 'ON':
    ...
    columns_output = ['Papel', 'EV/EBIT', 'Cotação', 'pos EV_EBIT', 'ROIC', 'pos ROIC',
        ...
        'Subsector','Timestamp', 'RANK','Papel','Cotação_last','Last Result Processed_last','Timestamp_last']
...
## ===== Feature added 26/08/2021 ====================
toggle = 'ON'
if toggle == 'ON':
    # Connect to SQLite database
    # !pip install sqlalchemy
    import sqlite3
    con = sqlite3.connect('MagicNumberdb.db')
    cur = con.cursor()

    from sqlalchemy import create_engine
    engine = create_engine('sqlite:///MagicNumberdb.db', echo=False)
    ranking.to_sql('ranking', con=engine, if_exists='append')
    try:
        print('exportado para o banco de dados com sucesso! timestamp: '+ranking['TimeStamp'][0])
    except:
        pass
...
# Next features intended
    ...
    ## DONE - get sector
    ## backlog - verify correlation
    ## backlog - verify dupplicated tickers tasa4 tasa3 (get the most liquidity one)
    ## DONE - get price
    ## DONE - from previous file - get price back then and see the delta variation of price
    ## DONE - get last position and the delta
    ## fazer aparecer a data do ultimo ranking

    ## Front end and db with flask and sqlite
    ### organize a flask directory
    ### save rankings database with timestamp in sql db
